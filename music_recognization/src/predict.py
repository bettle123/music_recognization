...
def predict_symbol(symbol_Info, filename, classes, model_dir, model_name):
    ...
    for i in range(0, len(symbol_Info)):
        ...
        '''
        ## Test Gaussian blurring effect on predictions
        ## Gaussian Blurring
        kernel_size = 2*2+1;
        crop_img= cv.GaussianBlur(crop_img,(kernel_size,kernel_size),0)
        ## Median Blurring
        kernel_size = 2*2+1;
        crop_img = cv.medianBlur(crop_img,kernel_size)
        ## denoise
        crop_img = cv.fastNlMeansDenoisingColored(crop_img,None,10,10,7,21)
        '''
        '''
        ## output every single sliding window with median kernel.
        ## Median Blurring
        kernel_size = 2*2+1;
        crop_img = cv.medianBlur(crop_img,kernel_size)
        store = "../Output/Predict_Output/0117_test/single/median/x_"+str(x_position)+"_y_"+str(y_position)+"_"+basefilename;
        cv.imwrite(store,crop_img);
        ## output every single sliding window with Morphological Transform- Opening.
        kernel_size = 5;
        kernel = np.ones((kernel_size,kernel_size),np.uint8)
        crop_img = cv.morphologyEx(crop_img, cv.MORPH_OPEN, kernel)
        store = "../Output/Predict_Output/0117_test/single/Open/x_"+str(x_position)+"_y_"+str(y_position)+"_"+basefilename;
        cv.imwrite(store,crop_img);
        '''
        kernel_size = 5;
        kernel = np.ones((kernel_size,kernel_size),np.uint8)
